...
if defined DEFAULT_MV_PROPERTYFILE (
   ...
   echo DEFAULT_MV_PROPERTYFILE is not defined using atavism.properties
   set DEFAULT_MV_PROPERTYFILE=atavism.properties
)
set PROPFILE=%1
if %1x==x (
...
if not defined DELETE_LOGS_ON_STARTUP (
  java -cp %MV_JAR% -Datavism.propertyfile=%PROPFILE% -Dwin_env_var=DELETE_LOGS_ON_STARTUP atavism.scripts.PropertyGetter atavism.delete_logs_on_startup > tmp.bat
  call tmp.bat
  del tmp.bat
)
...
if %1x==x (
	:: Set value of MV_WORLDNAME from atavism.worldname in property file
	java -cp %MV_JAR% -Datavism.propertyfile=%PROPFILE% -Dwin_env_var=MV_WORLDNAME atavism.scripts.PropertyGetter atavism.worldname > tmp.bat
	call tmp.bat
	del tmp.bat
) else (
    set MV_WORLDNAME=%1
...
set LOG4J=%MV_HOME%\other\log4j-1.2.15.jar
...
set JAVA_FLAGS=%JAVA_FLAGS% -cp %MV_CLASSPATH% -Datavism.logs=%MV_LOGS%

rmdir /S /Q %MV_HOME%\inject\atavism

java -Datavism.log_level=0 %JAVA_FLAGS% -ea atavism.server.marshalling.InjectClassFiles -m %MV_COMMON%\mvmarshallers.txt -m %MV_WORLD%\worldmarshallers.txt -t %MV_COMMON%\typenumbers.txt -i %MV_HOME%\build -o %MV_HOME%\inject\
